...
import { partyColorFromSlug, partyCssClassFromSlug, partyNameFromSlug } from "@/utils/Party";
...
export default async function ConstituencySummaryPage() {
  ...
  ).sort((a, b) => majority(a.impliedPreviousResult) - majority(b.impliedPreviousResult));
            ...
            <Row>
              <Col xs={12} lg={{ offset: 1, span: 10 }} xl={{ offset: 2, span: 8 }}>
                <Container className="text-center">
                  <Row xs={2}>
                    <Col><h5>We could turn this...</h5></Col>
                    <Col><h5>Into this...</h5></Col>
                  </Row>

                  {torySeats.map((c) => (
                    <Row xs={2} className="pb-1">
                      <Col>
                        <Link href={`/constituencies/${c.constituencyIdentifiers.slug}`}>
                          <span className="badge rounded-pill w-100 h-100 text-wrap align-middle" style={{ backgroundColor: partyColorFromSlug("Con") }}>
                      ...
                      <Col>
                        <Link href={`/constituencies/${c.constituencyIdentifiers.slug}`}>
                          <span className="badge rounded-pill w-100 h-100 text-wrap align-middle" style={{ backgroundColor: partyColorFromSlug(c.recommendation.partySlug) }}>
                            {(c.recommendation.partySlug ? partyNameFromSlug(c.recommendation.partySlug) : "TBC")}
                          </span>
                        </Link>
                      </Col>
                    </Row>
